...
final locationProvider = StreamProvider<Position>((ref) async* {
  ...
  LocationPermission permission = await Geolocator
      .checkPermission(); //checks current location permission status
  if (permission == LocationPermission.denied) {
    permission = await Geolocator
        .requestPermission(); //if permission is denied request permission using
    if (permission == LocationPermission.denied) {
  ...
  yield* Geolocator.getPositionStream(
    //Starts a stream that listens to location updates from the device.
